...
public abstract class PeerConnBase {
    ...
    public TcpConnServerInfo addPeerConn(ServerNodeInfo serverNodeInfo, ChannelHandlerContext context) {
        Map<String, TcpConnServerInfo> typeConnMap = serverTypeConnMap.computeIfAbsent(serverNodeInfo.getServerType(), (key) -> new HashMap<>());
        TcpConnServerInfo tcpConnServerInfo = typeConnMap.computeIfAbsent(serverNodeInfo.getServiceId(), (key) -> new TcpConnServerInfo(serverNodeInfo, 16));
