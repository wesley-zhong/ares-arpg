...
public class AresTcpClientConn implements InitializingBean {
    ...
    @Value("${netty.client.workerCount:2}")
    private int nettyClientWorkerCount;
    ...
    public void initWithMsgEncoder() {
        ...
        if (useLinux) {
            bossGroup = new EpollEventLoopGroup(nettyClientWorkerCount);
        } else {
            bossGroup = new NioEventLoopGroup(nettyClientWorkerCount);
        }
...
//                .option(ChannelOption.SO_KEEPALIVE, true)
    ...
    public ChannelFuture asynConnect(String ip, int port) {
