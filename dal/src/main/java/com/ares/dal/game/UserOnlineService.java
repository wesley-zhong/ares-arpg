...
public class UserOnlineService implements InitializingBean {
    ...
    private String SET_USER_TEAM_LUA = " local strDataObj = redis.call('get',KEYS[1]); \n" +
            " if(strDataObj ==  false) then   \n" +
            " return  nil   \n" +
            " end  \n" +
            " local existObj = cjson.decode(strDataObj);  \n" +
            " if tonumber(ARGV[1]) == 1  then\n" +
            " existObj.tmSrId=ARGV[2]\n" +
            " end \n" +
            " if tonumber(ARGV[1]) == 2 then \n" +
            "  existObj.targetId=ARGV[2]\n" +
            " end\n" +
    ...
    public UserOnlineStateDO resetTeamServId(long uid, String teamId) {
        String[] keys = new String[1];
        keys[0] = OLS + uid;
    ...
    public UserOnlineStateDO resetGamSrvId(long uid, String gmSrvId) {
        String[] keys = new String[1];
        keys[0] = OLS + uid;
        String[] args = new String[2];
    ...
    public UserOnlineStateDO resetTargetId(long uid, long targetId) {
        String[] keys = new String[1];
        keys[0] = OLS + uid;
        String[] args = new String[2];
        args[0] = "3";//1: change teamId  2:change targetId
        args[1] = targetId + "";
        Object changedResult = redisDAO.evalsha(SET_USER_TEAM_LUA_SHA, keys, args);
        String strUserOnlineStateDo = (String) ((List) changedResult).get(0);
        try {
    ...
    private void test() {
        ...
        resetGamSrvId(1002,"haha");
        resetTargetId(1002,10000);
    }
}
