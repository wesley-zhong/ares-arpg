...
@Slf4j
public class SyncRedisFactory extends RedisFactory {
    ...
    @Override
    public <T> T getAdnSet(String key, T obj) {
        try {
            String s = objectMapper.writeValueAsString(obj);
            String value = this.redisCommands.getset(key, s);
            if (value == null) {
                return null;
            }
            return (T) objectMapper.readValue(value, obj.getClass());
        } catch (Exception e) {
            log.error("---error", e);
        }
        return null;
