...
public class TimerManager {
    ...
    private void callTimeoutAction(TimerHandle handler) {
        try {
            ...
            LOGGER.error(NKStringFormater.format("timer-{} run failed.", handler), e);
        }
    }
    ...
    private void runOneTimer(TimerHandle handler) {
        try {
            ...
            LOGGER.error(NKStringFormater.format("{} run timer {} field.", name, handler), e);
        }
    }
    ...
    public int runTimer() {
        if (belongThreadId != 0 && belongThreadId != Thread.currentThread().getId()) {
            throw new IllegalStateException(NKStringFormater.format("timerManager-{} belongs to thread-{}, but run at thread-{}", name, belongThreadId, Thread.currentThread().getId()));
        }
        ...
        try {
            ...
            if (tickNum > TIMER_DELAY_WARNING && !firstRun) {
                LOGGER.warn(NKStringFormater.format("timerManager-{} tick delay num:{}, currentTick:{}, realTimeTick:{}", name, tickNum, getCurrentTick(), getRealTimeTick()));
            } else if (tickNum < 0 && tickNum % 3000 == 0) {
                LOGGER.error(NKStringFormater.format("realTimeTick {} less than logicTimeTick {}. maybe gm set system time before.", realTick, getCurrentTick()));
            }
            ...
            while (true) {
                ...
                if (handler.getExpireTick() < getCurrentTick()) {
                    LOGGER.error(NKStringFormater.format("timer-{} expireTick:{} < currentTick:{}", handler, handler.getExpireTick(), getCurrentTick()));
                } else if (handler.getExpireTick() > getCurrentTick()) {
                    throw new IllegalStateException(NKStringFormater.format("timer-{} expireTick:{} > currentTick:{}. FATAL ERROR, PLEASE SHUTDOWN SERVER!!!", handler, handler.getExpireTick(), getCurrentTick()));
                }
    ...
    private boolean setAccelerateRate(long newAccRate) {
        if (newAccRate < 0 || newAccRate > ACCELERATE_RATE_MAX) {
            throw new IllegalArgumentException(NKStringFormater.format("timerManager-{} accRate:{} not in range[0,{}]", this, newAccRate, ACCELERATE_RATE_MAX));
        }
        ...
        if (LOGGER.isInfoEnabled()) {
            LOGGER.info(NKStringFormater.format("timerManager-{} set accelerate rate set from {} to {}. tmpOffsetTimeMs:{}, totalOffsetTimeMs:{}, currentTick:{}, currentLogicTimeMs:{}, currentSystemTimeMs:{}"
    ...
    public void destroy() {
        LOGGER.warn(NKStringFormater.format("timerManager {} destroyed", name));
        isDestroyed = true;
    }
    ...
    private void checkAddTimerParam(boolean runForever, long periodTick) {
        if (belongThreadId != 0 && belongThreadId != Thread.currentThread().getId()) {
            throw new IllegalStateException(NKStringFormater.format("timerManager-{} belongs to thread-{}, but timer add in thread-{}",
                    name, belongThreadId, Thread.currentThread().getId()));
        }
        if (runForever && periodTick <= 0) {
            throw new IllegalStateException(NKStringFormater.format("forever timer period {} invalid", periodTick));
        }
    }
