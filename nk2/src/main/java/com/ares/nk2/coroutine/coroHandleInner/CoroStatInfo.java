...
public class CoroStatInfo {
    ...
    public void addCpuCostNs(long ns) {
        ...
        if (ns > warnNs) {
            CoroHandle.LOGGER.error(StringFormatter.format("cpucost>1000! {} unpark running for {}ms", jobName, nsToMs(ns)));
        } else {
            CoroHandle.LOGGER.warn(StringFormatter.format("{} unpark running for {}ms", jobName, nsToMs(ns)));
        }
    }
