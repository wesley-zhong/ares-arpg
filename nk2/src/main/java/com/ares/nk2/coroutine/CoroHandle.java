...
public class CoroHandle<V> {
    ...
    protected void onException(Throwable result) {
        if (result != null) {
            if (relationType == RelationType.runJob) {
                LOGGER.error(NKStringFormater.format("corohandle_runjob_exception_log, coroHandleId: {}", getCoroBaseInfo().coroHandleId()), result);
            }
        }
    }
    ...
    private void exception(Throwable t) {
        ...
        if (LOGGER.isInfoEnabled()) {
            LOGGER.info(NKStringFormater.format("coro create excep {}", t.getMessage()));
        }
    }
    ...
    private void addToWaiterList(CoroHandle<?> handle) {
        ...
        if (waiters.size() >= 1) {
            LOGGER.error(NKStringFormater.format("waitForMeJobList.size(): {} >= 1", waiters.size()));
        }
    ...
    public void onPinned(String reason) {
        LOGGER.error(NKStringFormater.format("fatalError coroutine pinned on {} of {} on thread {} reentranceid {}",
