...
public abstract class LocalService {
    ...
    class CallJobInitCallable implements Callable<Void> {
        ...
        public Void call() throws Exception {
            if (reloadState.get()) {
                throw new RuntimeException(NKStringFormater.format("callJobReload running, type: {}, executorServiceIndex: {}", getName(), executorServiceIndex));
            }
            try {
    ...
    public final int callJobReload() {
        ...
        if (ret == 0) {
            LOGGER.warn(NKStringFormater.format("callJobReload, ret: {}", ret));
        } else {
            LOGGER.warn(NKStringFormater.format("failed to callJobReload, ret: {}", ret));
        }
    ...
    public final int callJobStop() {
        LOGGER.warn(NKStringFormater.format("local service:[{}] callJobStop begin", getName()));
        ...
        for (int i = 0; i < getExecutorServiceCount(); i++) {
            try {
                ...
                LOGGER.error(NKStringFormater.format("local service:[{}] index:[{}] callJobStop failed! ", getName(), i), e);
            }
        }
        ...
        for (int i = 0; i < jobList.size(); i++) {
            ...
            try {
                ...
                ret = -1;
                LOGGER.error(NKStringFormater.format("local service:[{}] index:[{}] callJobStop timeout", getName(), i), e);
                break;
            }
            if (result == null) {
                ret = -1;
        ...
        if (ret == 0) {
            LOGGER.warn(NKStringFormater.format("local service:[{}] callJobStop end, ret: {}", getName(), ret));
        } else {
            LOGGER.warn(NKStringFormater.format("local service:[{}] callJobStop failed, ret: {}", getName(), ret));
        }
        ...
        try {
            ...
            for (CoroHandle<Integer> handle : resultArr) {
                try {
                    ...
                    LOGGER.error(NKStringFormater.format("local service callJobProc error, jobName: {}", jobName), e);
                }
            }
    ...
    public enum DoNotDirectCall {
        ...
        public <V> V callJob(LocalService localService, long hashKey, long timeout, Callable<V> call, String jobname) {
            ...
            if (executor == null) {
                throw new RuntimeException(NKStringFormater.format("failed to get executor, hashKey: {}", hashKey));
            }
