...
public class CoroutineSch {
    ...
    public CoroHandle<?> pollReadyJob() {
        ...
        if (!readyJobList.isEmpty()) {
            ...
            if (handle == null) {
                LOGGER.error(StringFormatter.format("null handle found {}", readyJobList.first().getData().coroutineName()));
                ...
                if (handle.getCoroBaseInfo().getStatus() != CoroHandle.CoroJobStatus.READY) {
                    LOGGER.error(StringFormatter.format("poll unready job {}", handle));
                    return pollReadyJob();
                }
            }
            return handle;
        ...
        try {
            ...
            for (IntrusiveList.IntrusiveElem<CoroWorker> iter = untimedParkList.first(); iter != untimedParkList.end(); iter = iter.next()) {
                ...
                if (parkTime > 0 && (DateUtils.currentTimeMillis() >= (parkTime + CoroutineConfig.maxMsUntimedCoroutinePark))) {
                    LOGGER.error(StringFormatter.format("Coro {}:{} id {} handleid {} park for more than {} ms use jstack to check the stack"
            ...
            LOGGER.error(StringFormatter.format("CoroContainer {} procTotal Error", container.coroThread.getName()), e);
        }
    }
...
//        }
            ...
            while (true) {
                try {
                    ...
                    if (readyJobList.contains(worker.listElem)) {
                        LOGGER.error(StringFormatter.format("worker error {} {} {} {} {}"
    ...
    protected void checkHandleRunningStatus(CoroHandle handle) {
        ...
        if (parkPerSec > 1) {
            LOGGER.warn(StringFormatter.format("job {} coroid {} park {} times per seconds for {} secs"
