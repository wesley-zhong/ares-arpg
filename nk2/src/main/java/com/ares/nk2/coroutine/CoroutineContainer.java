...
public class CoroutineContainer {
    ...
    public final void startContainer() {
        LOGGER.error(NKStringFormater.format("startContainer coroThread state:{}", coroThread.getState()));
        coroThread.start();
    }
...
//        }
    ...
    public class MainVtJob implements Runnable {
        ...
        @Override
        public void run() {
            LOGGER.error(NKStringFormater.format("container name {} {}", getMainCoroName(), Thread.currentThread().getName()));
            try {
                coroInit();
                while (isRunning) {
                    try {
            ...
            LOGGER.error(NKStringFormater.format("coroThread {} stopped", getCoroThread().getName()));
        }
