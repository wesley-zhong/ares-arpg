...
public class NoAsyncCheck implements AutoCloseable {
    ...
    public boolean end() {
        ...
        CoroHandle currentCoroHandle = CoroHandle.current();
        if (currentCoroHandle == null) {
            LOGGER.error(StringFormatter.format("NoAsyncCheck.end called not in coroutine env, caller: {}", FunctionUtil.getCallerInfo(0)));
            return false;
        }

        if (currentCoroHandle != coroHandle) {
            LOGGER.error(StringFormatter.format("NoAsyncCheck.begin CoroHandle != NoAsyncCheck.end CoroHandle, caller: {}", FunctionUtil.getCallerInfo(0)));
            return false;
        }
        return true;
    }
    ...
    public void onPark() {
        LOGGER.error(StringFormatter.format("NoAsyncCheck park, stack trace: {}", ThreadStateMonitor.getStackTraceSelfClass()));
        coroHandle.setNoAsyncCheck(null);
        System.exit(-1);
    }
