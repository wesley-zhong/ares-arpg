...
public abstract class ExecutorServiceUtil {
    ...
    public static ScheduledExecutorService newSingleThreadScheduledExecutor(String threadName, boolean forceShutdown, boolean ignore) {
        if (executorServiceMap.containsKey(threadName)) {
            LOGGER.error(StringFormatter.format("already has used the thread name {}", threadName));
            return null;
        }
    ...
    public static void shutdownGraceful(Class<?> clazz, List<ExecutorService> executorServices, int timeoutInSeconds) {
        ...
        while (true) {
            if (executorServices.isEmpty()) {
                LOGGER.error(StringFormatter.format("{} shutdownGraceful ok", clazz.getSimpleName()));
                break;
            }
            ...
            while (iterator.hasNext()) {
                ...
                LOGGER.error(StringFormatter.format("await termination {} total= {}", clazz.getSimpleName(), executorServices.size()));
            }
        }
    }
    ...
    public static void shutdownGraceful() {
        ...
        while (true) {
            ...
            while (it.hasNext()) {
                ...
                ref = entry.getValue();
                if (ref == null) {
                    LOGGER.error(StringFormatter.format("{} finished ref == null", entry.getKey()));
                    it.remove();
                ...
                instance = ref.get();
                if (instance == null) {
                    LOGGER.error(StringFormatter.format("{} finished instance == null", entry.getKey()));
                    it.remove();
                    noForceThreads.remove(entry.getKey());
                    continue;
                }
                try {
                    if (true == instance.awaitTermination(1, TimeUnit.SECONDS)) {
                        LOGGER.error(StringFormatter.format("{} finished, cost {} ms", entry.getKey(), (DateUtils.currentTimeMillis() - startTime)));
                        it.remove();
                        noForceThreads.remove(entry.getKey());
                        continue;
                    }
                ...
                LOGGER.error(StringFormatter.format("await termination total= {} {}", executorServiceMap.size(), noForceThreads));
            }
        }
    ...
    public static void shutdownAndAwaitTermination(String threadName) {
        ...
        if (ref == null || ref.get() == null) {
            LOGGER.error(StringFormatter.format("not found ExecutorService {}", threadName));
            return;
        }
        ...
        if (es == null) {
            LOGGER.error(StringFormatter.format("es == null ExecutorService {}", threadName));
            return;
        }
        try {
            es.shutdown();
            ...
            while (es.awaitTermination(5, TimeUnit.SECONDS) == false) {
                ...
                if (now >= breakTimeout) {
                    LOGGER.error(StringFormatter.format("shutdownAndAwaitTermination timeout {}", threadName));
                    break;
                }
                LOGGER.error(StringFormatter.format("shutdown {}...", threadName));
            }
