...
public class PlayerLoginService {
    ...
    public void gamePlayerInnerLogin(long uid, ProtoInner.InnerGameLoginRequest gameInnerLoginRequest) {
        ...
        playerCreateRet.targetId = gameInnerLoginRequest.getTargetId();
        //cache player info
    ...
    private void playerLoginOnDataLoaded(long sid, PlayerCreateRet playerCreateRet) {
        ...
        player.start();
        player.login(playerCreateRet.isNew, playerCreateRet.loadFromMem, playerCreateRet.targetId, false);
        teamService.onPlayerLogin(player.getUid());
