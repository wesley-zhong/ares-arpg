...
@Component
@Slf4j
public class SceneMgr implements InitializingBean {
    @Autowired
    ...
    Map<Integer, Scene> scene_map = new HashMap<>();
    private List<SceneInfo> scene_list = new ArrayList<>();

    private void init(){
        createScene(Scene.DEFAULT_SCENE_NAME, 0, false, null);
    }
    ...
    Scene getSceneBySceneID(int scene_id)
    {
        return scene_map.get(scene_id);
    }

    Scene getSceneByActorID(long actor_id)
    {
        return getSceneBySceneID(actor_scene_map.get(actor_id));
    }

    public void createScene(String scene_name, long create_actor_id, boolean reset_data, ProtoScene.CreateSceneRes.Builder message)
    {
        ...
        ProtoScene.CreateSceneFinishNtf.Builder builder = ProtoScene.CreateSceneFinishNtf.newBuilder();
        builder.setSceneId(scene_id);
        getPeerConn().sendGateWayMsg(create_actor_id, ProtoCommon.MsgId.CREATE_SCENE_FINISH_NTF_VALUE, builder.build());
    }
    ...
    public void enterScene(int scene_id, long actorid, int profession)
    {
        ...
        Scene scene = getSceneBySceneID(scene_id);
        if (scene == null)
        {
    ...
    public void enterDefaultScene(long actorid, int profession)
    {
        ...
        Scene scene = getSceneByActorID(actorid);
        if (scene == null){
    ...
    {
        Scene scene = getSceneByActorID(actor_id);
