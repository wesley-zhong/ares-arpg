...
import com.ares.game.scene.entity.*;
...
public abstract class Scene extends SceneModuleContainer {
    ...
    static class ScenePlayerLocation
    {
        Vector3 curPos;           // 当前位置
        Vector3 curRot;           // 当前朝向
        Vector3 lastValidPos;    // 最后一次处于正常状态(站、走、跑)的坐标
        Vector3 lastValidRot;    // 最后一次处于正常状态(站、走、跑)的朝向
    };
...
//        getBlockGroupComp().erasePlayerReloadPos(uid);
    ...
    ScenePlayerLocation getPlayerLocation(long uid)
    {
        return playerLocationMap.get(uid);
    }

    public Transform getPlayerCurrentLocation(long uid)
    {
        ScenePlayerLocation location = playerLocationMap.get(uid);
        if (location == null) {
            return null;
        }
        return new Transform(location.curPos, location.curRot);
    }
...
//        }
    ...
    public void playerPreEnter(Player player)
    {
        ...
        {
            // 更新host_player
            Player host_player = getHostPlayer();
            if (host_player == null)
            {
        ...
        if (peerMap.size() >= MAX_PLAYER_COUNT)
        {
            ...
            throw new LogicException(ProtoCommon.ErrCode.SCENE_IS_FULL_VALUE);
        }
...
//        }
    ...
    // 玩家进入场景
    public void playerEnter(Player player, Vector3 pos, Vector3 rot, boolean isReLogin, List<Avatar> enterSceneAvatarList, Avatar appearAvatar)
    {
...
//        }
        ...
        long uid = player.getUid();
        int peer_id = getPeerId(uid);
        if (0 == peer_id)
        {
            throw new UnknownLogicException("peer_id not exist in scene, scene_id:" + sceneId + ", uid:" + uid);
        }
...
//        auto cur_team_entity = player.getAvatarComp().getTeamEntity();
//        if (cur_team_entity == null || cur_team_entity.getScene() != shared_from_this())
//        {
//            if (0 != addAvatarTeamEntity(player))
//            {
//                log._WARNING + "addAvatarTeamEntity fails" + player;
//                return -1;
//            }
//        }

//        vector<int64_t> team_avatar_guid_vec;
//        for (auto avatar : enterSceneAvatarList)
//        {
//            if (avatar == null)
//            {
//                log._ERROR + "avatar is null" + player;
//                continue;
//            }
//            team_avatar_guid_vec.emplace_back(avatar.getGuid());
//        }
//
//        // 警告代码, 上一个角色未被回收！
//        Avatar cur_avatar = player.getCurAvatar();
//        if (cur_avatar != null  cur_avatar.isOnScene())
//        {
//            log._ERROR + "[ENTER]last cur avatar is on scene!, uid:" + uid;
//        }
        // 设置cur_scene
        Scene curScene = player.getSceneModule().getCurScene();
...
//        auto old_guid_vec = scene_team.getAvatarGuidVec(uid);
//        RefreshAbilityGuard guard;
//        for (auto guid : team_avatar_guid_vec)
//        {
//            guard.addCreature(player.getAvatarComp().findAvatar(guid));
//        }
//        if (0 != scene_team.setPlayerAvatarTeamAndAddToScene(uid, team_avatar_guid_vec, appear_avatar.getGuid(), *this, getChangeSceneTeamReason(), false))
//        {
//            log._ERROR + "setPlayerAvatarTeamAndAddToScene failed, uid:" + uid;
//            return -1;
//        }
//        auto now_guid_vec = scene_team.getAvatarGuidVec(uid);
//        for (auto guid : old_guid_vec)
//        {
//            // 在队伍中, 跳过
//            if (std::find(now_guid_vec.begin(), now_guid_vec.end(), guid) != now_guid_vec.end())
//            {
//                continue;
//            }
//            Avatar avatar = player.getAvatarComp().findAvatar(guid);
//            // 不在队伍中
//            // 尝试从当前场景踢走
//            if (avatar != null  avatar.getScene() == this_scene)
//            {
//                if (avatar == player.getCurAvatar())
//                {
//                    log._ERROR + "try delete avatar is cur avatar, this should not happen, uid:" + player.getUid() + " avatar:" + *avatar;
//                    continue;
//                }
//                if (this_scene != null)
//                {
//                    this_scene.delAvatarAndWeaponEntity(*avatar);
//                }
//            }
//        }
...
//        }
        ...
        log.info("[EnterScene] player Enter scene, scene_id:" + sceneId + ", peer_id:" + peer_id + ", uid:" + uid);
    }
...
//        // 清理TeamEntity
//        if (0 != delAvatarTeamEntity(player))
//        {
//            log._WARNING + "delAvatarTeamEntity fails" + player;
//        }
//        // 清理生成器
...
//        delPlayTeamEntity(player);
    ...
    void notifyAllPlayerInfo()
    {
        ...
        // 一人时也要发送
        notifyAllPlayer(ProtoCommon.MsgId.SCENE_PLAYER_INFO_NTF, notify.build(), 0);
    }

    // 将协议发送给所有玩家
    void notifyAllPlayer(ProtoCommon.MsgId msgId, Message message, long excludeUid)
    {
        for (Player player : playerMap.values())
        {
...
//#endif
        ...
        if (entity_type == ProtoScene.ProtEntityType.PROT_ENTITY_AVATAR)
        {
            ...
            ScenePlayerLocation player_location = new ScenePlayerLocation();
            player_location.curPos = avatar.getPosition();
            player_location.curRot = avatar.getRotation();
            player_location.lastValidPos = avatar.getLastValidPos();
            player_location.lastValidRot = avatar.getLastValidRot();
            setPlayerLocation(uid, player_location);
            avatar.setLastValidPos(player_location.lastValidPos);
            avatar.setLastValidRot(player_location.lastValidRot);
        }
        ...
        VisionContext appearContext = context;
...
//        }
    ...
    {
//        AvatarTeamEntity team_entity = EntityMgr::createAvatarTeamEntity(*this);
//        if (team_entity == null)
//        {
//            log._WARNING + "team_entity is null" + player;
//            return -1;
//        }
//        if (0 != addGridlessEntity(*team_entity))
//        {
//            log._WARNING + "addGridlessEntity fails" + *team_entity;
//            return -1;
//        }
//        team_entity.setScene(toThis(*this));
//        team_entity.setPlayer(toThis(player));
//        team_entity.setAuthorityPeerId(getPeerId(player.getUid()));
//        player.getAvatarComp().setTeamEntity(team_entity);
    }

    // 删除角色队伍实体
    public void delAvatarTeamEntity(Player player)
    {
//        AvatarTeamEntity team_entity = player.getAvatarComp().getTeamEntity();
//        if (team_entity == null)
//        {
//            log._WARNING + "team_entity is null" + player;
//            return -1;
//        }
//        // 退出场景时，删除队伍实体
//        int team_entity_id = team_entity.getEntityId();
//        player.getAvatarComp().setTeamEntity(null);
//        if (0 != delGridlessEntity(team_entity_id))
//        {
//            log._WARNING + "delGridlessEntity fails" + *team_entity;
//            return -1;
//        }
//        notifyDelTeamEntity(team_entity_id);
    }
...
//#endif
        ...
        player.sendMessage(ProtoCommon.MsgId.SCENE_ENTITY_APPEAR_NTF, appearNtf.build());
...
//#endif
        ...
        player.sendMessage(ProtoCommon.MsgId.SCENE_ENTITY_DISAPPEAR_NTF, notify.build());
    }
    ...
    public void notifyViewingPlayersExclude(Entity entity, ProtoCommon.MsgId msgId, Message msg, long excludeUid, boolean includeMe)
    {
...
//        SceneExcelConfig* scene_config_ptr = getConfig();
//        if (scene_config_ptr == nullptr)
//        {
//            LOG_WARNING << "getConfig fails";
//            return -1;
//        }
//        if (!scene_config_ptr->level_entity_config.empty())
//        {
//            scene_entity_ptr_->addInitLevelConfig(scene_config_ptr->level_entity_config);
//        }
    }
