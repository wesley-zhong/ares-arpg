...
//        }
    ...
    public void toClient(ProtoScene.SceneEntityInfo.Builder pb) {
        ...
        getMotionInfo(pb.getMotionInfoBuilder());
        pb.setLastMoveSceneTimeMs(lastMoveSceneTimeMs);
    }

    // 获取当前的移动信息
    public void getMotionInfo(ProtoScene.MotionInfo.Builder motionInfo)
    {
...
//            is_move_sent_reliable_ = true;
    ...
    void broadcastMotionInfo(ProtoScene.MotionInfo motionInfo, MotionContext motionContext)
    {
        ...
        ProtoScene.SceneEntityMoveNtf.Builder notify = ProtoScene.SceneEntityMoveNtf.newBuilder();
        notify.setEntityId(entityId);
        notify.setMotionInfo(motionInfo);
        notify.setSceneTime(motionContext.sceneTimeMs);
        scene.notifyViewingPlayersExclude(this, ProtoCommon.MsgId.SCENE_ENTITY_MOVE_NTF, notify.build(), motionContext.excludeUid, true);
    }

    // 获取上一次的移动通知协议
    void getLastMoveNotify(ProtoScene.SceneEntityMoveNtf.Builder notify)
    {
        notify.setEntityId(entityId);
        getMotionInfo(notify.getMotionInfoBuilder());
        notify.setSceneTime(lastMoveSceneTimeMs);
    }
    ...
    public void notifyLastMotionInfo(Player player)
    {
        ...
        if (ownerPlayer == player)
            return;
        ProtoScene.SceneEntityMoveNtf.Builder notify = ProtoScene.SceneEntityMoveNtf.newBuilder();
        getLastMoveNotify(notify);
        player.sendMessage(ProtoCommon.MsgId.SCENE_ENTITY_MOVE_NTF, notify.build());
    }
