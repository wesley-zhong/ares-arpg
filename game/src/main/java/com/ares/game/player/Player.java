...
public class Player extends PlayerModuleContainer {
    ...
    private final PlayerBasicModule basicModule;
    private final PlayerSceneModule sceneModule;
    private final PlayerItemModule itemModule;
    private final PlayerAvatarModule avatarModule;

    public Player(long id, PeerConn peerConn) {
        this.uid = id;
        this.peerConn = peerConn;

        basicModule = new PlayerBasicModule(this);
        sceneModule = new PlayerSceneModule(this);
        itemModule = new PlayerItemModule(this);
        avatarModule = new PlayerAvatarModule(this);
    }
...
//            setIsQuitMp(false);
    ...
    public Avatar getCurAvatar(){
        return avatarModule.getCurAvatar();
    }

    // 玩家初始化，然后通知进入场景
    private void initAndBeginEnterScene(boolean reLogin, long targetUid, boolean clientReLogin) {
        ...
        if (targetUid != 0 && targetUid != uid)
        {
            ...
            try {
                sceneModule.joinPlayerScene(targetUid);
                return;
            }
            catch (FyLogicException e) {
                log.error("joinPlayerScene failed. uid " + uid + " targetUid " + targetUid, e);
...
//                        }
        ...
        beginEnterScene(scene, reLogin, clientReLogin);
    }


    // 开始进入场景
    private void beginEnterScene(Scene scene, boolean reLogin, boolean clientReLogin)
    {
...
//            log.DEBUG + "findRebornPositionRotation succ, uid:" + getUid();
        ...
        {
            transform = scene.getPlayerValidLocation(uid);
        }

        beginEnterScene(scene, transform, enterType, ProtoScene.VisionType.VISION_MEET, reLogin, clientReLogin, ProtoScene.EnterReason.ENTER_REASON_LOGIN);
    }

    public void beginEnterScene(Scene destScene, Transform transform, ProtoScene.EnterType enterType,
                         ProtoScene.VisionType visionType, boolean reLogin, boolean clientReLogin, ProtoScene.EnterReason enterReason)
    {
        if (destScene == null)
        {
        ...
        PlayerSceneModule.EnterSceneState enterSceneState = sceneModule.getEnterSceneState();
        ...
        // 记录传送目标
        markDestination(destScene, transform, enterType, visionType, reLogin, clientReLogin);
        // 发送进场景通知
        ...
        if (curScene != null)
        {
            ...
            curScene.getPlayerValidLocation(uid).getPosition().toClient(notify.getPrevPosBuilder());
        }
...
//            }
        ...
        sendMessage(ProtoCommon.MsgId.PLAYER_ENTER_SCENE_NTF, notify.build());

        sceneModule.setEnterSceneState(PlayerSceneModule.EnterSceneState.ENTER_SCENE_NOTIFY);

        log.info("[EnterScene] BeginEnterScene uid:" + getUid() + " enterType:" + enterType
            + " token:" + notify.getEnterSceneToken() + " destSceneId:"
            + notify.getSceneId() + " pos:" + notify.getPos() + " targetUid:" + notify.getTargetUid());
    }
    ...
    public ProtoScene.EnterSceneReadyRes enterSceneReady(ProtoScene.EnterSceneReadyReq req)
    {
        ...
        if (req.getEnterSceneToken() != sceneModule.getEnterSceneToken())
        {
            ...
            throw new FyLogicException(ProtoCommon.ErrCode.ENTER_SCENE_TOKEN_INVALID_VALUE);
        }
...
//        }
        ...
        try {
            ...
            if (ex instanceof FyLogicException) {
                throw (FyLogicException) ex;
            }
...
//        }
        ...
        sendMessage(ProtoCommon.MsgId.ENTER_SCENE_PEER_NTF, notify.build());
    ...
    void disappearInCurScene()
    {
        ...
        if (curAvatar.getScene() == curScene)
        {
            curAvatar.leaveScene(VisionContext.MISS, 0);
        }

        PlayerViewMgr playerViewMgr = curScene.findPlayerViewMgr(getUid());
        if (playerViewMgr != null)
        {
            playerViewMgr.resetPlayerViewMgr();
        }
        else
        {
            log.error("uid:{} view_mgr is null", getUid());
        }
    }
    ...
    void leaveCurScene()
    {
        ...
        {
            log.warn("[EnterScene] getCurAvatar failed, uid:" + getUid());
        }
    ...
    void leaveDestroyedWorld()
    {
        ...
        // 遍历所有角色 清理一下
        List<Avatar> avatarList = getAvatarModule().getAvatarList();
    ...
    private void markDestination(Scene destScene, Transform transform, ProtoScene.EnterType enterType,
                                 ProtoScene.VisionType visionType, boolean reLogin, boolean clientReLogin)
    {
        ...
        sceneModule.setEnterReLogin(reLogin);
        sceneModule.setClientReLogin(clientReLogin);
    }
    ...
    public ProtoScene.SceneInitFinishRes sceneInitFinish(ProtoScene.SceneInitFinishReq req)
    {
        ...
        playerEnterScene(destScene, destPos, destRot, enterType, visionType, enterReLogin);
    ...
    void playerEnterScene(Scene destScene, Vector3 pos, Vector3 rot, ProtoScene.EnterType enterType,
                             ProtoScene.VisionType visionType, boolean isReLogin)
    {
        if (destScene == null)
        {
...
//        sceneModule.notifyAllMarkPoint();
    ...
    public ProtoScene.EnterSceneDoneRes enterSceneDone(ProtoScene.EnterSceneDoneReq req)
    {
        ...
        PlayerViewMgr viewMgr = curScene.findPlayerViewMgr(getUid());
        if (viewMgr == null)
        {
            throw new UnknownLogicException("[EnterScene] viewMgr is null, uid:" + getUid());
        }
        viewMgr.notifyEyePointState();
...
//        curScene.getMultistagePlayComp().onPostEnterScene(*this);
    ...
    {
        InitEnterSceneAvatarRet ret = new InitEnterSceneAvatarRet();
        ret.enterSceneAvatarList = getAvatarModule().getAvatarList();
        ret.appearAvatar = getCurAvatar();
        return ret;
    }
