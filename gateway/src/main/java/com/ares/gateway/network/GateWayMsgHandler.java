...
public class GateWayMsgHandler implements AresServerTcpHandler {
    ...
    public void handleMsgRcv(AresTKcpContext aresTKcpContext) throws IOException {
        ...
        if (calledMethod != null) {
            ...
            if (useCoroutine) {
                PlayerLocalService.getInstance().processNetworkMessage(hashCode, aresTKcpContext, calledMethod, uid, paraObj, msgHeader);
            } else {
        ...
        if (playerSession != null) {
            ...
            log.info("------------------ msgId={} redirect to game",msgHeader.getMsgId());
        }
        else {
